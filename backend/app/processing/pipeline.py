...
def _process_one(payload: dict):
    ...
    with get_sync_session() as session:
        existing = session.query(Ioc).filter(Ioc.value == value, Ioc.ioc_type == ioc_type).first()
        ...
        if existing:
            ...
            ioc = Ioc(ioc_type=ioc_type, value=value, status=IocStatus.ACTIVE, first_seen=now, last_seen=now)
            session.add(ioc)
            session.flush()
        if source_id:
            existing_src = session.query(IocSource).filter(
                IocSource.ioc_id == ioc.id, IocSource.source_id == source_id).first()
            if not existing_src:
                session.add(IocSource(ioc_id=ioc.id, source_id=source_id, seen_at=now, raw_data=raw_data))
                session.flush()
        source_count = session.query(IocSource).filter(IocSource.ioc_id == ioc.id).count()
        ioc.score = compute_score(ScoreContext(source_count=max(source_count, 1), feed_weight=1.0, ioc_type=ioc_type))
        session.commit()
